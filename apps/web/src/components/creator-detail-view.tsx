...
import { type CreatorView } from "@/lib/data";
...
export function CreatorDetailView({ creator }: CreatorDetailViewProps) {
  ...
  const avgViews = creator.recentVideos.length
    ? Math.round(
        creator.recentVideos.reduce((sum, v) => sum + v.views, 0) /
          creator.recentVideos.length
      )
    : 0;
  const avgEngagement = creator.recentVideos.length
  ...
  // Projection data
  const minDeal = creator.minimumDealSize || 1;
  ...
  const sentimentSummary = `This creator's audience demonstrates strong purchase intent, particularly in ${(creator.categories[0] ?? "content").toLowerCase()}-related products. Comments frequently mention specific products, ask for recommendations, and express willingness to buy. Interest categories align well with ${creator.categories.join(", ")} verticals. Overall sentiment is highly positive (94% positive, 4% neutral, 2% negative).`;
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-xs font-medium text-gray-500">
                Top {creator.nicheRanking.percentile}% in{" "}
                {creator.nicheRanking.category}
              </p>
            </div>
        ...
        <div className="rounded-card bg-white p-6 shadow-card">
          ...
          <div className="space-y-3">
            <ScoreBar
              ...
              score={creator.score.topicRelevance}
              weight="25% weight"
            />
            <ScoreBar
              label="Recent Views"
              score={creator.score.recentViews}
              weight="20% weight"
            />
            <ScoreBar
              label="Engagement Health"
              score={creator.score.engagementHealth}
              weight="18% weight"
            />
            <ScoreBar
              label="Authenticity"
              score={creator.score.authenticity}
              weight="15% weight"
            />
            <ScoreBar
              label="Activity / Consistency"
              score={creator.score.activityConsistency}
              weight="12% weight"
            />
            <ScoreBar
              label="Comment-Audience Match"
              score={creator.score.commentAudienceMatch}
              weight="10% weight"
            />
          </div>
        </div>
              ...
              <div className="flex items-center gap-2">
                <div className="h-2 w-24 overflow-hidden rounded-full bg-gray-100">
                  <div
                    className="h-full rounded-full bg-emerald-500"
                    style={{
                      width: `${creator.authenticity.likeToViewNormality}%`,
                    }}
                  />
                </div>
                <span className="text-sm font-semibold text-gray-800">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-surface-100">
                  ...
                  <th className="pb-3 text-right">
                    <SortHeader field="engagementRate" label="Engagement" />
                  </th>
                </tr>
              </thead>
